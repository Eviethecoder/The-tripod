...
class HealthBar extends FlxSpriteGroup
{
	...
	public var middlebaBar:FlxSprite;
	...
	public function new(x:Float, y:Float, image:String = 'healthBar', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1)
	{
		...
		switch (image){
			...
			case('healthBarog'):
				barWidth = Std.int(bg.width - 250); //the variables that cause pain
				barHeight = Std.int(bg.height - 123);
				trace (barWidth);
	...
	public function updateBar()
	{
		...
		switch (hpbar){
			...
			case('healthBarog'):
				leftBar.setPosition(bg.x +140, bg.y +60);
				rightBar.setPosition(bg.x +140, bg.y +60);
